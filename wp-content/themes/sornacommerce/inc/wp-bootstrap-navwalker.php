	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
				...
				foreach ( $atts as $attr => $value ) {
				 ...
				 if ( in_array( 'current-menu-item', $classes, true ) ) {
					$attributes .= 'class="general active"';
				 }else{
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html( 'Add a menu ( Primary / Main Menu )', 'sornacommerce' ) . '</a></li>';
...
/* Check if Class Exists. */
if ( ! class_exists( 'WP_Bootstrap_Navwalker_Responsive' ) ) {
	/**
	 * WP_Bootstrap_Navwalker class.
	 *
	 * @extends Walker_Nav_Menu
	 */
	class WP_Bootstrap_Navwalker_Responsive extends Walker_Nav_Menu {
		/**
		 * Start Level.
		 *
		 * @see Walker::start_lvl()
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			* a 0 if the strings are equal.
			...
			if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
				...
				$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
				$item_output .= ( $args->has_children && 0 === $depth ) ? '</a>  <a class=" drop-toggle" href="#"><i class="fa fa-angle-down"></i></a>' : '</a>';
				$item_output .= $args->after;
				$output .= apply_filters( 'walker_nav_menu_start_el', $item_output, $item, $depth, $args );
			}
		}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				echo '>';
				echo '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="">' . esc_html( 'Add a menu ', 'sornacommerce' ) . '</a></li>';
				echo '</ul>';
				if ( $container ) {
					echo '</' . esc_attr( $container ) . '>'; }
			}
